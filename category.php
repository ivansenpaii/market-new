...
<?php
    ...
    include "connect.php";
        ...
        $pagecount = 4;  //кол-во записей на странице
       ...
       //404 
        $count_query = mysqli_query($link,"SELECT MAX(category_id) FROM category__product");
        $count_row = mysqli_fetch_row($count_query);
        $count = $count_row[0]; 
        $new_url = '/404.php';
        if ($_GET['id'] > $count || $_GET['id']<=0) {
